...
list_ds = tf.data.Dataset.from_tensor_slices(temp_images).map(tf_load_images, num_parallel_calls=128)

detected_images = []
detected_image_paths = []
for converted_img, path in list_ds:
    detected_images.append(detector1(converted_img)['detection_class_entities'].numpy())
