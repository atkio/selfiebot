...
flags.DEFINE_string(
    "saved_model_path",
    "AIScript/model/mobilenet_v2_140_224",
    "The model to load.")
...
def main(args):
    ...
    if FLAGS.image_dir is None or not os.path.exists(FLAGS.image_dir):
        raise ValueError(
            "image_dir must be a valid directory with images to classify.")
    ...
    images, image_paths = load_images(
        FLAGS.image_dir, (FLAGS.image_dim, FLAGS.image_dim))
    ...
    for f in images_preds:
        if float(images_preds[f]["porn"]) > 0.7:
            shutil.move(f, "./DEL/")
        if float(images_preds[f]["sexy"]) > 0.7:
            shutil.move(f, "./SEXY/")
